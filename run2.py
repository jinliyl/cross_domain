...
def train_test(cross, part, model, name, op_step):
    if cross:
        cross_train = train_nn(
            ...
            target_dic_path = "pre_data/Sina_dic",
            source_dic_path = "pre_data/QQ_dic",
            target_path = "pre_data/Sina_index",
            source_path = "pre_data/QQ_index",
            transfer_path = "pre_data/QQ--Sina_trans",
            part = part,
            model = model,
            batch_size = 32,
            num_filters = 128,
            ...
            embedding_dim = 128,
            op_step = op_step,
            word_vec_target = "pre_data/Sina_vectors", 
            word_vec_source = "pre_data/QQ_vectors",
            tf_df_target = "pre_data/Sina_tf_df",
            tf_df_source = "pre_data/QQ_tf_df",
            max_iter = 500)
...
if __name__ == "__main__":
    ...
    for x in [4, 8, 16, 32, 64]:
        res = []
        para = [False, x, "bi_lstm", "Sina", 2e-4]
        time_str = datetime.now().isoformat()
        f = open("./result/" + time_str, "w")
        for i in range(10):
            print(str(i) + " times training...")
