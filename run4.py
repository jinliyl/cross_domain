...
def train_test(cross, part, model, name, op_step):
    if cross:
        cross_train = train_nn(
            ...
            target_dic_path = "pre_data/Reddit_pw_dic",
            source_dic_path = "pre_data/Reddit_st_dic",
            target_path = "pre_data/Reddit_pw_index",
            source_path = "pre_data/Reddit_st_index",
            transfer_path = "pre_data/Reddit_st--pw_trans",
            part = part,
            model = model,
            batch_size = 32,
            num_filters = 128,
            ...
            embedding_dim = 128,
            op_step = op_step,
            word_vec_target = "pre_data/Reddit_pw_vectors_re", 
            word_vec_source = "pre_data/Reddit_st_vectors_re",
            tf_df_target = "pre_data/Reddit_pw_tf_df_re",
            tf_df_source = "pre_data/Reddit_st_tf_df_re",
            max_iter = 900)
...
if __name__ == "__main__":
    ...
    for x in [4, 8, 16, 32, 64]:
        res = []
        para = [True, x, "bi_lstm", "Reddit_pw", 4e-4]
    ...
    for x in [4, 8, 16, 32, 64]:
        res = []
        para = [False, x, "cnn", "Reddit_pw", 1e-3]
        time_str = datetime.now().isoformat()
        f = open("./result/" + time_str, "w")
        for i in range(10):
            print(str(i) + " times training...")
    ...
    for x in [4, 8, 16, 32, 64]:
        res = []
        para = [False, x, "bi_lstm", "Reddit_pw", 4e-4]
        time_str = datetime.now().isoformat()
        f = open("./result/" + time_str, "w")
        for i in range(10):
            print(str(i) + " times training...")
